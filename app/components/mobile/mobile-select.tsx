...
type Props = { onSelect: (loc: LocationRow) => void };

export default function MobileSelect({ onSelect }: Props) {
  ...
  return (
    <div className="md:hidden">
      <label htmlFor="mobile-select" className="block text-xs uppercase tracking-wider text-neutral-400 mb-2">
        Auswahl
      </label>
      ...
      <div className="relative">
        <select
          ...
          className="block w-full h-10 bg-[#181818] text-white border border-[#2A2A2A] px-3 pr-10
        ...
        <span className="pointer-events-none absolute right-3 top-1/2 -translate-y-1/2 text-white/70 select-none">▾</span>
      </div>
    </div>
  );
}
